...
func (sc *txSourceImpl) Fixture() (*oasis.NetworkFixture, error) {
	...
	f.Runtimes[1].Constraints[scheduler.KindComputeExecutor][scheduler.RoleWorker].MinPoolSize.Limit = uint16(f.Runtimes[1].Executor.GroupSize)
	f.Runtimes[1].Constraints[scheduler.KindComputeExecutor][scheduler.RoleBackupWorker].MinPoolSize.Limit = uint16(f.Runtimes[1].Executor.GroupBackupSize)
